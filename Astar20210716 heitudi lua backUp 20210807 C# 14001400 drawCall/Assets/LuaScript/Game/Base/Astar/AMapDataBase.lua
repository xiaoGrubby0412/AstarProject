...
function AMapDataBase:GetMapDataType(x, y)
    ...
    elseif self.blockData[x] ~= nil and self.blockData[x][y] == AMapDataType.Block then
        return AMapDataType.Block
    else
...
function AMapDataBase:IsBlock(x, y)
    if x > self.mapWidth - 1 or x < 0 or y > self.mapHeight - 1 or y < 0 then
        ...
        return self.blockData[x] ~= nil and self.blockData[x][y] == AMapDataType.Block
    end
end

function AMapDataBase:SetBlock(x, y)
    if x > self.mapWidth - 1 or x < 0 or y > self.mapHeight - 1 or y < 0 then
        print("in AMapData SetBlock out bounds Error x == " .. x .. " y == " .. y)
        return
    end
...
function AMapDataBase:SetNormal(x, y)
    if x > self.mapWidth - 1 or x < 0 or y > self.mapHeight - 1 or y < 0 then
        print("in AMapData SetNormal out bounds Error x == " .. x .. " y == " .. y)
        return
    end
    self.blockData[x] = self.blockData[x] or {}
    self.blockData[x][y] = nil
end
