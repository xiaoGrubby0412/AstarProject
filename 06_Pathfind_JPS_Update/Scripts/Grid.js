function Grid() {
    ...
    for (var i = 0; i < cols; i++) {
        ...
        for (var j = 0; j < rows; j++) {
            this.spots[i][j] = new Spot(i, j, int(random(1, 5)));
        }
    }
    this.drections = [];
    this.drections[0] = { x: 0, y: -1 };// 下
    this.drections[1] = { x: 1, y: -1 };// 右下
    this.drections[2] = { x: 1, y: 0 }; // 右
    this.drections[3] = { x: 1, y: 1 }; // 右上
    this.drections[4] = { x: 0, y: 1 }; // 上
    this.drections[5] = { x: -1, y: 1 };// 左上
    this.drections[6] = { x: -1, y: 0 };// 左
    this.drections[7] = { x: -1, y: -1 };// 左下
}
